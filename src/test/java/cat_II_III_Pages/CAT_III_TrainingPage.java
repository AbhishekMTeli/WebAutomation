...
public class CAT_III_TrainingPage {
	...
	public boolean cat_III_lHSRemarksIsEnabled() {
		try {
			...
			wait.until(ExpectedConditions.visibilityOf(cat_III_LHSRemarksTextAreaField));

			// Check if element is enabled (HTML attribute)
			boolean enabled = cat_III_LHSRemarksTextAreaField.isEnabled();

			// Check CSS classes that indicate disabled appearance
			String classAttr = cat_III_LHSRemarksTextAreaField.getAttribute("class");
			boolean visuallyDisabled = classAttr != null && classAttr.contains("disabled");

			// Check for CSS property that disables pointer events
			String pointerEvents = cat_III_LHSRemarksTextAreaField.getCssValue("pointer-events");
	...
	public boolean cat_III_rHSRemarksIsEnabled() {
		try {
			WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(timeout));
			wait.until(ExpectedConditions.visibilityOf(cat_III_RHSRemarksTextAreaField));

			// Check if element is enabled (HTML attribute)
			boolean enabled = cat_III_RHSRemarksTextAreaField.isEnabled();

			// Check CSS classes that indicate disabled appearance
			String classAttr = cat_III_RHSRemarksTextAreaField.getAttribute("class");
			boolean visuallyDisabled = classAttr != null && classAttr.contains("disabled");

			// Check for CSS property that disables pointer events
			String pointerEvents = cat_III_RHSRemarksTextAreaField.getCssValue("pointer-events");
			boolean pointerEventsNone = "none".equalsIgnoreCase(pointerEvents);
	...
	public boolean rhsCAT_III_GradeIsEnabled(String section, String gradeNumber) {
	...
	public boolean lhsCAT_III_GradeIsEnabled(String section, String gradeNumber) {
		String gradeXpath = getLHSGradingCell(section, gradeNumber);
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(timeout));
		try {
			WebElement gradeButton = wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(gradeXpath)));
